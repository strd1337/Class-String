...
String operator + (const char *str, const String & other)
{
	...
	//очищаем память, выделеная под \0 в кнтр по умолчанию
	delete tmp.pointer;
	
	//длина 
	tmp.p_length = other.p_length + strlen(str);
	
	//память 
	tmp.pointer = new char [tmp.p_length + 1];
	
	//копируем строку
	strcpy(tmp.pointer, str);
	
	//соединяем с объектом
	strcat(tmp.pointer, other.pointer);
